...
class SimpleWaypointFollowingLocalPlanner(LocalPlanner):
    ...
    def run_step(self, vehicle: Vehicle) -> VehicleControl:
        ...
        num_points = 3 #take the average of the next number of waypoints as a new target
        lst = [self.way_points_queue[i] for i in range(num_points)]
        target_waypoint = self.average(lst)
    ...
    def average (self, lst):
        "Return the average of the waypoints in a list"

        x,y,z= 0,0,0

        for element in lst:
            x += element.get_matrix()[0, 3]   #get x axis of the Location
            y += element.get_matrix()[1, 3]   #get y axis of the Location
            z += element.get_matrix()[2, 3]   #get z axis of the Location

        x,y,z = x/len(lst),y/len(lst),z/len(lst)

        return Transform(location=Location(x=x,y=y,z=z), rotation= Rotation(pitch=0,yaw=0,roll=0))
